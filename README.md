...
如需日常使用，推荐双击 `control_panel.cmd` 打开控制台：日报智能体与 DSH Web 可分别启用或关闭。启用智能体只启动日报调度，不会打开 DSH Web；启用后可直接最小化窗口，日报任务仍由 Windows 在后台执行。控制台会显示日报任务、数据服务、日报页面、DSH Web 以及当前安装或更新操作的实时状态；下方运行日志会逐行显示环境检测、Git、npm、pip、插件安装和服务启停命令的执行进度。
...
日报推送配置的 `market` 默认为 `"a"`；设为 `"hk"` 即生成港股交易日简报，并且只分析 `market: "hk"` 的持仓：
...
5. 日报2会逐项检查日报1候选和持仓结论：买入后从上午快照跌幅不高于 -2%，或卖出后涨幅不低于 2%，记为“明显背离”；其他结果分为“相符”“待观察”或“数据不足”。
...
可以点击控制台中的“更新项目”，或双击项目根目录的 `update_agent.cmd`。更新会先停止全部项目服务和日报任务，再执行 `git pull --ff-only`，更新 `.venv` 中的 Python 依赖，并重新注册本地 `dsh-finance-agent` 插件；不会更新 dsh 或 pnpm。更新完成后所有服务保持关闭，由用户按需重新启用。`config/daily-push.json`、持仓、历史日报和反思数据均不会被 Git 覆盖。
