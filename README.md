...
可以点击控制台中的“更新项目”，或双击项目根目录的 `update_agent.cmd`。更新会先停止全部项目服务和日报任务，再执行 `git pull --ff-only`，并重新安装 Python 依赖与 DSH 插件。更新完成后所有服务保持关闭，由用户按需重新启用。`config/daily-push.json`、持仓、历史日报和反思数据均不会被 Git 覆盖。
