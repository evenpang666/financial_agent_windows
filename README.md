...
如需日常使用，推荐双击 `control_panel.cmd` 打开控制台：日报智能体与 DSH Web 可分别启用或关闭。启用智能体只启动日报调度，不会打开 DSH Web；启用后可直接最小化窗口，日报任务仍由 Windows 在后台执行。控制台会显示日报任务、数据服务、日报页面、DSH Web 以及当前安装或更新操作的实时状态。
