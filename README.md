...
如需日常使用，推荐双击 `control_panel.cmd` 打开控制台：日报智能体与 DSH Web 可分别启用或关闭。启用智能体只启动日报调度，不会打开 DSH Web；启用后可直接最小化窗口，日报任务仍由 Windows 在后台执行。

首次使用仍需先运行一次 `install-and-start.ps1`，以创建 Python 环境并安装 dsh。若首次只想准备日报功能、不打开 DSH Web，可运行 `./install-and-start.ps1 -NoDshWeb`；之后日常启停均可通过控制台完成。
